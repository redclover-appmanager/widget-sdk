...
### Added
- **Custom Data Support**: Added `data-custom-data` attribute support to the script tag.
  - This allows passing an optional string for additional context (e.g., a specific form ID for a multi-form app).
  - The value is automatically included in the authorization request and the generated JWT.
  - It is also automatically included in all analytics events tracked via `this.track()`.
